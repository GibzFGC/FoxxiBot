const chatEle = document.getElementById('log');
...
const bttvEmoteCache = {
  lastUpdated: 0,
  data: { global: [] },
  urlTemplate: '//api.betterttv.net/3/cached/emotes/global/{{id}}/{{image}}' };
...
function showMessage({ chan, type, message = '', data = {}, timeout = 0, attribs = {} } = {}) {
  ...
  chatLine.classList.add('messageBox');
  chatLine.setAttribute('style', "--user-color: " + data.color);
  
  let chatMeta = document.createElement('span');
  chatMeta.classList.add('meta');

  chatMain.appendChild(chatLine);
  chatLine.appendChild(chatMeta);

  if (chan) {
    chatMain.setAttribute('channel', chan);
  }

  Object.keys(attribs).
  forEach(key => {
    chatMain.setAttribute(key, attribs[key]);
  });

  if (type === 'chat') {
    'id' in data && chatMain.setAttribute('message-id', data.id);
    'user-id' in data && chatMain.setAttribute('user-id', data['user-id']);
    'room-id' in data && chatMain.setAttribute('channel-id', data['room-id']);
    'username' in data && chatMain.setAttribute('username', data.username);
    ...
    let cubeEle = document.createElement('div');

    if (cube == "cube2") {
      cubeEle.classList.add('cube1');
      cubeEle.innerText = "";
      cube = "cube1";
    } else {
      cubeEle.classList.add('cube2');
      cubeEle.innerText = "";
      cube = "cube2";
    }
  ...
  chatEle.appendChild(chatMain);

  setTimeout(() => chatMain.classList.add('visible'), 100);
  ...
  if (timeout) {
    setTimeout(() => {
      if (chatMain.parentElement) {
        chatMain.classList.remove('visible');
        setTimeout(() => chatEle.removeChild(chatMain), 1000);
      }
    }, timeout);
  }
}
  ...
  data.forEach(n => {
    ...
    } else
    {
      ...
      if (type === 'bttv') {
        ...
        url = url.replace('{{id}}', n.id).replace('{{code}}', '1x');
...
function getBTTVEmotes(channel) {
  ...
  let global = true;
  if (channel) {
    endpoint = 'users/twitch/11969896';
    global = true;
  }
  return request({
